...
class MyCustomDelegate extends SearchDelegate<String> {
  ...
  late GetSavedSearchBloc getSavedSearchBloc = GetSavedSearchBloc(GetSavedSearchProvider());
  ...
  ThemeData appBarTheme(BuildContext context) {
    return ThemeData(
      ...
      inputDecorationTheme: InputDecorationTheme(
        ...
        isDense: true,
        contentPadding: EdgeInsets.only(left: 16.0, bottom: 8.0, top: 8.0, right: 8.0),
        filled: true,
  ...
  Widget? buildLeading(BuildContext context) {
    return GestureDetector(
      child: Icon(Icons.arrow_back_outlined, color: Colors.black, size: 30.0,),
      onTap: (){
  ...
  Widget buildResults(BuildContext context) {
    //todo: làm kết quả khi ấn search ở đây nhé, truy cập thẳng biến 'query' để lấy keyword
    return Text('result nè: ' + query);
  }
        ...
        child: BlocBuilder<GetSavedSearchBloc, GetSavedSearchState>(
            ...
            builder: (context, state){
              if(state is GetSavedSearchState && listSavedSearch != null){
                List<ItemSavedSearchModel> listToShow = <ItemSavedSearchModel>[];
                if (query.isNotEmpty) {
